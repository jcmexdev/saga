...
func (s *GRPCOrderService) CreateOrder(
	...
	// 1. Mapeamos items de entity -> proto
	...
	// 2. Construimos el request gRPC
	req := &orderv1.CreateOrderRequest{
		CustomerId: customerID,
		Items:      protoItems,
	}

	// 3. Pasar la idempotency key como metadata (si viene)
	if idempotencyKey != "" {
		md := metadata.Pairs("x-idempotency-key", idempotencyKey)
	...
	// 5. Mapear proto.Order -> entity.Order
...
func mapProtoOrderToEntity(po *orderv1.OrderInfo) *entity.Order {
	return &entity.Order{
		...
		CreatedAt:  "", // igual, si quieres, luego lo agregamos al proto
		UpdatedAt:  "",
	}
}
